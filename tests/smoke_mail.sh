...
mb_json="$("$ROOT_DIR/scripts/commands/mailbox/list.sh" "${first_acc:-}" 2>&1)" || { echo "smoke_mail: Mail not running, skipping."; exit 0; }
printf '%s\n' "$mb_json" | "$JQ_BIN" -e 'type == "array"' >/dev/null || { echo "smoke_mail: mailbox list is not JSON array." >&2; exit 1; }
