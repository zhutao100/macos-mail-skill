...
find_envelope_index() {
  ...
  # Prefer the common modern location when present.
  local preferred="$HOME/Library/Mail/V10/MailData/Envelope Index"
  if [[ -f "$preferred" ]]; then
    printf '%s\n' "$preferred"
    return 0
  fi

  # Fallback: search a small depth under ~/Library/Mail.
  find "$base_dir" -maxdepth 5 -type f -name 'Envelope Index' -print -quit 2>/dev/null
}
